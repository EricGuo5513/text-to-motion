...
'''For use of training baseline [ICCV 2021]'''
class Text2MotionDatasetICCV2019(data.Dataset):
    ...
    def __getitem__(self, item):
        ...
        if m_length != self.max_length:
            ...
            s_idx = 0
        # (seq_len, joint_num, 3)
        # Get local positions, offset the root position
        motion[:, 1:] = motion[:, 1:] - motion[:, 0:1]
        motion[1:, 0] = motion[1:, 0] - motion[:-1, 0]
        ...
        # print(word_embeddings.shape, motion.shape)
        # print(tokens)
        return caption, src_motion, tgt_motion, m_length
...
class MotionDataset(data.Dataset):
    def __init__(self, opt, mean, std, split_file):
        ...
        for name in tqdm(id_list):
            motion = np.load(pjoin(opt.motion_dir, name + '.npy'))
            if motion.shape[0] < opt.unit_length * 2:
                continue
            self.lengths.append(motion.shape[0] - opt.unit_length * 2)
    ...
    def __getitem__(self, item):
        if item != 0:
            ...
            motion_id = 0
            idx = 0
        motion1 = self.data[motion_id][idx:idx+self.opt.unit_length]
        motion2 = self.data[motion_id][idx+self.opt.unit_length : idx+self.opt.unit_length*2]
        "Z Normalization"
